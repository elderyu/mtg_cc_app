class CollectedCardsController < ApplicationController
  ...
  def create
    if params[collected_cards_create_path][:count].nil?
  ...
  def add_face_to_collection face_name
    ...
    if found_card = CollectedCard.find_by(user_id: current_user.id, card_id: collected_card.id)
      found_card.update(count: (found_card.count + params[collected_cards_create_path][:count].to_i))
    else
      current_user.add_card collected_card
      CollectedCard.find_by(user_id: current_user.id, card_id: collected_card.id).update(count: params[collected_cards_create_path][:count])
    end
    flash[:success] = "#{params[:name]} successfully added to your collection! Total number of copies: #{params[collected_cards_create_path][:count]}"
  end
end
